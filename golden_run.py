...
def main() -> int:
    ...
    cutout_op = _find_op(ops, layer="PLAYER_CUTOUT")
    if not cutout_op:
        _die(f"Plan missing required op for layer PLAYER_CUTOUT (expected cutout set_image).")

    if cutout_op.get("op") != "set_image":
        _die(f"PLAYER_CUTOUT op must be set_image. Got: {cutout_op.get('op')}")

    meta = cutout_op.get("meta") or {}
    if meta.get("image_type") != "smart_object":
        _die(f"PLAYER_CUTOUT meta.image_type must be smart_object. Got: {meta.get('image_type')}")

    cutout_value = cutout_op.get("value")
    if not isinstance(cutout_value, str) or not cutout_value:
        _die("PLAYER_CUTOUT value must be a non-empty string path.")

    # Resolve relative asset paths against repo root.
    cutout_path = (repo_root / cutout_value).resolve() if not os.path.isabs(cutout_value) else Path(cutout_value)
    if not cutout_path.exists():
        _die(f"Cutout asset referenced in plan does not exist: {cutout_path}")

    toggle_op = _find_op(ops, layer="HAS_PLAYER_CUTOUT")
    if not toggle_op:
        _die("Plan missing required toggle for HAS_PLAYER_CUTOUT.")

    if toggle_op.get("op") != "toggle":
        _die(f"HAS_PLAYER_CUTOUT op must be toggle. Got: {toggle_op.get('op')}")

    toggle_value = toggle_op.get("value")
    if toggle_value is not True:
        _die(f"HAS_PLAYER_CUTOUT value must be true. Got: {toggle_value!r}")

    # Text contract assertions (minimum viability for result template)
    required_text_layers = [
        "TEXT_WINNER",
        "TEXT_LOSER",
        "TEXT_WINNER_SCORE",
        "TEXT_LOSER_SCORE",
    ...
    print("Verified: PLAYER_CUTOUT=set_image smart_object + HAS_PLAYER_CUTOUT=true")
